						...
						<table class="table table-hover text-dark">
							<thead class="thead-dark">
								<tr>
									...
									<th scope="col-3">Thời gian</th>
									<th scope="col-1">Chủ phòng</th>
									<th scope="col-1">Người đặt</th>
									<th scope="col"></th>
								</tr>
							</thead>
							...
							<tbody>
								<c:forEach items="${dhChoxn}" var="dh">
									<tr>
										...
										<td class="col-2 text-center">${dh.phong.ten }</td>
										<td class="col-2 text-center">${dh.phong.gia}</td>
										<td class="col-3 text-center">${hanhdong==2?dh.phong.dcchitiet:""}
											${dh.phong.xa.tenxa }, ${dh.phong.xa.huyen.tenhuyen },
											${dh.phong.xa.huyen.tinh.tentinh }</td>
										<td class="col-3 text-center">${dh.ngay }<br>${dh.gio }</td>
										<td class="col-1 text-center"><a
											href="<c:url value="/admin/taikhoan?id_tk=${dh.phong.taikhoan.id_tk }"/>">${dh.phong.taikhoan.tentk }</a></td>
										<td class=" col-1 text-cneter"><a
											href="<c:url value="/admin/taikhoan?id_tk=${dh.nguoidat.id_tk }"/>">${dh.nguoidat.tentk }</a></td>
										<c:if test="${dh.trangthai == 1 }">
											<td><a
												href="<c:url value='/admin/ql-dathen/xoa?id_p=${dh.id_p }&id_tk=${dh.id_tk }&id_dh=${dh.id_dh }'/>">Xóa</a></td>
										</c:if>
									</tr>
								</c:forEach>
							</tbody>
