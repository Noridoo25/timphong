...
public class VerifyController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(trueCode.equals(currentCode)) {
			boolean check =  taikhoanService.insertTaikhoan((Taikhoan)session.getAttribute("taikhoan"));
			if(check)
			{
			...
			{
				req.setAttribute("notverifysuccess", "Đã có lỗi xảy ra khi tạo tài khoản");
				req.getRequestDispatcher("/views/web/verify.jsp").forward(req, resp);
			}
		}
		else {
